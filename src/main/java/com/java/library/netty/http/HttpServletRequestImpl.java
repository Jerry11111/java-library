...
import com.java.library.netty.http.HttpServer.RequestLine;
...
public class HttpServletRequestImpl implements HttpServletRequest{
	...
	public int getContentLength() {
		String value = headers.get(KEY_CONTENT_LENGTH);
	...
	public String getContentType() {
		return headers.get(KEY_CONTENT_TYPE);
	}
	...
	public ServletInputStream getInputStream() throws IOException {
		ServletInputStreamImpl impl = new ServletInputStreamImpl(socket.getInputStream());
		return impl;
	}
	...
	public void doRead(){
		...
		try {
			...
			if(contentLength > 0) {
				...
				in.read(data);
				paramaters = RequestLine.parseUrlQuery(new String(data));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String getParameter(String name) {
		if(requestLine.paramaters != null) {
			String value = requestLine.paramaters.get(name);
			if(value != null) {
				return value;
			}
		}
